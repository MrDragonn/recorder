...
-(void)drawRect:(CGRect)rect{
    ...
    for (NSNumber* number in self.array) {
        ...
        if(y<=1) y = 1;
        aPoints[0] =CGPointMake(x,self.frame.size.height/2- self.frame.size.height*y*1.5/160);//坐标1+25
        aPoints[1] =CGPointMake(x, self.frame.size.height/2+self.frame.size.height*y*1.5/160);//坐标2-25
   ...
   CGContextDrawPath(context, kCGPathStroke);
    ...
    CGPoint lline[2];
    lline[0]= CGPointMake(0, 20);
    lline[1]= CGPointMake(self.frame.size.width, 20);
